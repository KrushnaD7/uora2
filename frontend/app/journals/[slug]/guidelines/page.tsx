...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `Author Guidelines — ${name}`,
    description: `Instructions for preparing and submitting a manuscript to ${name}.`,
  };
}

export default async function JournalGuidelinesPage({ params }: PageProps) {
  const { slug } = await params;
  const res = await serverGetPublicJournalBySlug<JournalModel>(slug);
  ...
  const customGuidelines = journal.settings?.guidelines?.trim();
        ...
        <Container width="wide">
          ...
          <div className="mt-8 max-w-3xl">
            ...
            <p className="mt-5 text-[17px] leading-8 text-ink-600">
              Instructions for preparing and submitting your manuscript to {journal.name}.
            </p>
          </div>
        </Container>
      </section>
        ...
        <Container width="narrow">
          {customGuidelines ? (
            <div className="space-y-10">
              <p className="whitespace-pre-line leading-8 text-ink-600">{customGuidelines}</p>
            ...
            <div className="space-y-10">
              ...
              <section className="space-y-3">
                ...
                <p className="leading-8 text-ink-600">
                  By submitting a manuscript, the authors declare that the work is original, is not under consideration elsewhere, has been approved by all co-authors, and that all funding sources and conflicts of interest are disclosed.
                </p>
              </section>
