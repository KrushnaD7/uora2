...
// API base URL. Defaults to the same-origin `/api` path (the Vite dev server
// proxies it to the local backend; in production the web server routes it to
// the Node API). Set VITE_API_URL to a full URL when the API lives on another
// origin, e.g. https://api.uorapublications.com.
const API_URL =
