...
export default function FeaturedJournals() {
  // Was an async server component; the SPA fetches the same data in the
  // browser. The section stays hidden until it has journals to show, which
  // matches the original behaviour when the request returned nothing.
  const { data: res, loading } = useAsyncData(
    () => serverGetPublicJournals<JournalModel[]>(),
    []
  );
