...
// Code-split the heavy console the same way next/dynamic did, with an
// identical placeholder so the layout does not shift while it loads.
const EditorialConsole = lazy(() => import("./EditorialConsole"));
const editorialConsoleFallback = (
      ...
      <Container width="wide">
        <div className="grid items-center gap-14 pb-16 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.02fr)] lg:gap-16 lg:pb-24 xl:gap-20">
          ...
          <Reveal delay={0.18} className="relative mx-auto w-full max-w-[34rem] lg:max-w-none">
            <Suspense fallback={editorialConsoleFallback}>
              <EditorialConsole />
            </Suspense>
          </Reveal>
        </div>
